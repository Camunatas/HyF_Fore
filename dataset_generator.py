' THIS SCRIPT HAS BEEN MODIFIED TO JUST CREATE A .NPY FILE WITH ELECTRICITY PRICE DATA'
#   Output structure:
...
Pgen_dataset = np.load('Pgen_pred.npy', allow_pickle=True).item()
windspe_dataset = np.load('windspe_real.npy', allow_pickle=True).item()
...
while day != pd.Timestamp(ending_day) + pd.Timedelta('1d'):
    ...
    # Storing DM prices
    # Daily_dict['Price_pred_DM'] = Price_pred_DM_dataset[daily_key]
