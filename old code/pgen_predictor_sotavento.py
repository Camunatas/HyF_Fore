...
# Plotting 2018 historical data for paper
dataset_raw[df_day_start_ts:df_day_end_ts]['Speed'].astype(float).plot(xlabel=' ', ylabel='Wind speed (m/s)')
#%% Study correlations
import statsmodels.api as sm
sm.graphics.tsa.plot_acf(dataset_raw[df_day_start_ts:df_day_end_ts]['Speed'].astype(float).values)
plt.show()
...
while day != day_end_ts + pd.Timedelta('1h'):
    ...
    for i, hour in enumerate(hours):
        ...
        # # Declare model: ARIMA
        # # model = sm.tsa.SARIMAX(SARIMA_train, order=model_order, initialization='approximate_diffuse')
        # print('Fit start')
...
# TEMPORAL: Plotting prediction vs real
# fig = plt.figure()
# plt.plot(windspe_pred, label='pred')
...
#%% Saving results
np.save('windspe_real.npy', Pgen_pred_dict)